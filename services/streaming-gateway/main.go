...
<body>
  ...
  <script>
    ...
    const KEY_MAP = {
      ...
      'ShiftLeft':134,'ShiftRight':134,
      'KeyZ':122,'KeyX':120,'KeyC':99,'KeyV':118,
      'KeyB':98,'KeyN':110,'KeyM':109,
      'Comma':44,'Period':46,'Slash':47,
      'ControlLeft':133,'ControlRight':133,
      'AltLeft':132,'AltRight':132,'Space':32,
      'ArrowUp':128,'ArrowDown':129,'ArrowLeft':130,'ArrowRight':131,
      'Escape':27,
      'F1':135,'F2':136,'F3':137,'F4':138,'F5':139,
      'F6':140,'F7':141,'F8':142,'F9':143,'F10':144,'F11':145,'F12':146,
      'Insert':147,'Delete':148,'PageDown':149,'PageUp':150,'Home':151,'End':152
    };
    ...
    const MOUSE_BTN = { 0: 200, 2: 201, 1: 202 };
    canvas.addEventListener('mousedown', (e) => {
      if (document.pointerLockElement && ws.readyState === 1) {
        const qk = MOUSE_BTN[e.button];
        if (qk !== undefined) ws.send(JSON.stringify({type:'input',kind:'key',key:qk,down:true}));
      }
    });
    canvas.addEventListener('mouseup', (e) => {
      if (document.pointerLockElement && ws.readyState === 1) {
        const qk = MOUSE_BTN[e.button];
        if (qk !== undefined) ws.send(JSON.stringify({type:'input',kind:'key',key:qk,down:false}));
      }
    });
